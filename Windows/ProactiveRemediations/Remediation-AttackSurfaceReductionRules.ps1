...
<#
      .SYNOPSIS
      Intune remediation script for Attack surface reduction rules

      .DESCRIPTION
      Intune remediation script for Attack surface reduction rules

      .EXAMPLE
      PS C:\> .\Remediation-Windows10ASR.ps1
      ...
      .NOTES
      Simple Intune remediation script
#>
...
foreach ($RulesId in $RulesIds)
{
   if ($AttackSurfaceReductionRules.GUID -contains $RulesId)
   {
            ...
            }).AttackSurfaceReductionRules_Actions)
      {
         ...
         {
            # MAKE IT SO
            $null = (Add-MpPreference -AttackSurfaceReductionRules_Ids $AttackSurfaceReductionRules.GUID -AttackSurfaceReductionRules_Actions (($AttackSurfaceReductionRules | Where-Object -FilterScript {
                        $PSItem.GUID -eq $RulesId
                     }).Action) -ErrorAction SilentlyContinue -Force)
         }
         catch
         {
